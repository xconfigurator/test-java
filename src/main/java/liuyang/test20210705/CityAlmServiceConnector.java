package liuyang.test20210705;

import liuyang.test20210705.common.Vendors;
import liuyang.test20210705.common.connector.ConnInfo;
import liuyang.test20210705.common.connector.ServiceConnector;
...
public class CityAlmServiceConnector extends ServiceConnector<CityAlmInterface> {
    ...
    public Optional<CityAlmInterface> adaptVendor(ConnInfo connInfo) {
        ...
        if (Vendors.HBFEC == connInfo.getVendor()) {
            result = getPort(wsdlLocation, "liuyang.test20210705.adapter.hbfec.CityAlmServiceAdapter");
        }

        if (Vendors.HYTERA == connInfo.getVendor()){
            result = getPort(wsdlLocation, "liuyang.test20210705.adapter.hytera.CityAlmServiceAdapter");
        }

package liuyang.questions.prototype20210705.archived;

import liuyang.questions.prototype20210705.CityAlmInterface;
import liuyang.questions.prototype20210705.stub.hbfec.CityAlmService;
...
public class CityAlmInterfaceTest {
    ...
    void testPortHbfec() {
        ...
        Optional<CityAlmInterface> port = cityAlmServiceConnector.getPort("http://xxxx1/CityAlmService.wsdl", "liuyang.questions.test20210705.adapter.hbfec.CityAlmServiceAdapter");
        if (port.isPresent()) {
    ...
    void testPortHytera() {
        CityAlmServiceConnector<CityAlmInterface> cityAlmServiceConnector = new CityAlmServiceConnector<CityAlmInterface>();
        Optional<CityAlmInterface> port = cityAlmServiceConnector.getPort("http://xxxx2/CityAlmService.wsdl", "liuyang.questions.test20210705.adapter.hytera.CityAlmServiceAdapter");
        if (port.isPresent()) {
            String foo = port.get().foo("hello! ");
            log.info("存根可配置化：" + foo);
        }
